...
function drawObj(obj) {
    // ctx.beginPath(); ctx.rect(obj.x, obj.y, obj.size, obj.size); ctx.fillStyle =
    // obj.color; ctx.fill(); ctx.closePath();
...
// function inEnemy(px, py, enemy) {   return (enemy.x < px && px < enemy.x +
// enemy.size) &&     (enemy.y < py && py < enemy.y + enemy.size) } function
// collison(enemy) {   if (inEnemy(player.x, player.y, enemy)) return true;
...
// else if (inEnemy(player.x + player.size, player.y + player.size, enemy))
// return true;   else return false; }
function collison(enemy) {
    if (player.x < enemy.x + enemy.size && player.y < enemy.y + enemy.size && player.x + player.size > enemy.x && player.y + player.size > enemy.y) {
        return true;
    }
    return false;
}
